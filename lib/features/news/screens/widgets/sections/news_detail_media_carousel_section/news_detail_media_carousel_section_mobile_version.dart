...
class NewsDetailMediaCarouselSectionMobileVersion extends StatefulWidget {
  final NewsDetailStore newsDetailStore;

  const NewsDetailMediaCarouselSectionMobileVersion({
    super.key,
    required this.newsDetailStore,
  });

  @override
  State<NewsDetailMediaCarouselSectionMobileVersion> createState() =>
      _NewsDetailMediaCarouselSectionMobileVersionState();
}

class _NewsDetailMediaCarouselSectionMobileVersionState
    extends State<NewsDetailMediaCarouselSectionMobileVersion> {
  ...
  Widget build(BuildContext context) {
    return Observer(
      builder: (context) {
        if (widget.newsDetailStore.mediaLoading) {
        ...
        if (widget.newsDetailStore.mediaError != null) {
          return Padding(
            padding: const EdgeInsets.fromLTRB(24, 16, 24, 0),
            child: ListErrorState(
              message: 'Não foi possível carregar as imagens.',
              onRetry: widget.newsDetailStore.refreshMedia,
        ...
        final media = widget.newsDetailStore.media.toList();
