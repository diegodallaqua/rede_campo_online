...
class PendingProjectMedia {
  final XFile file;
  String name;

  PendingProjectMedia({required this.file, this.name = ''});
}

class ProjectMediaUploadField extends StatefulWidget {
  const ProjectMediaUploadField({
    super.key,
  ...
  final List<ProjectMedia> existingItems;
  final List<PendingProjectMedia> items;
  ...
  @override
  State<ProjectMediaUploadField> createState() =>
      _ProjectMediaUploadFieldState();
}

class _ProjectMediaUploadFieldState extends State<ProjectMediaUploadField> {
  final _picker = ImagePicker();
  final Map<PendingProjectMedia, TextEditingController> _controllers = {};
  final Map<PendingProjectMedia, Future<Uint8List>> _thumbnails = {};

  TextEditingController _controllerFor(PendingProjectMedia item) {
  ...
  Future<Uint8List> _thumbnailFor(PendingProjectMedia item) {
    return _thumbnails.putIfAbsent(item, () => item.file.readAsBytes());
  }

  @override
  void didUpdateWidget(ProjectMediaUploadField oldWidget) {
...
class _ExistingMediaItem extends StatelessWidget {
  ...
  Widget _placeholder() => Container(
        ...
        child: const Icon(Icons.image_outlined,
            size: 22, color: Color(0xFFBBBBAA)),
      );
}
